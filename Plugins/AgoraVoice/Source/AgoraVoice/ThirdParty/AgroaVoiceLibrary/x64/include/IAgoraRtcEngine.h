...
//        return setObject("rtc.video.set_remote_video_stream", "{\"uid\":%u,\"stream\":%d}", uid, streamType);
    ...
    */
    ...
    int startScreenCapture(HWND windowId, int captureFreq, const Rect *rect) {
        if (!rect)
            return setObject("che.video.start_screen_capture", "{\"id\":%u,\"captureFreq\":%d}", (/*unsigned int)windowId*/ unsigned _int64(windowId) , captureFreq));
        else
            return setObject("che.video.start_screen_capture", "{\"id\":%u,\"captureFreq\":%d,\"top\":%d,\"left\":%d,\"bottom\":%d,\"right\":%d}", /*(unsigned int)windowId*/ unsigned _int64(windowId) , captureFreq, rect->top, rect->left, rect->bottom, rect->right);
    }
