...
//DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FWebSocketReceiveByteData, const uint8* data,uint32 dataLen);


//enum websocket state
enum E_WebSocket_State
{
    closed,
    error,
    connectComplete
...
typedef enum E_WebSocket_State EWS_State;

typedef std::function< void(const char*, uint32) > WebSocketOnMessage;
typedef std::function< void(EWS_State, const char*,uint32) > WebSocketOnEvent;
...
struct SendBinaryStruct
{
    char* sendData;
    uint32 dataLen;

//     inline SendBinaryStruct& operator=(const SendBinaryStruct& from) {
// 
//         return *this;
//     }
};

typedef struct SendBinaryStruct MXBinaryStruct;
...
public:
    ...
    void SendByteData(const char* data,uint32 len);

    void SendDataBinary(MXBinaryStruct& data);
    ...
    //get event : closed,connected,error,etc
    void ProcessWebSocketEvent(EWS_State wsState, const char* in,int len);
...
public:
    ...
    void SetWebSocketCallfunc(WebSocketOnMessage _Mes_callfunc,WebSocketOnEvent _Eve_callfunc);

    bool GetWSConnected() { return (mlws == nullptr ? false : true); }

private:    
    std::list<MXBinaryStruct> m_SendQueue_byte;

    WebSocketOnMessage _OnMessage_Callfunc;
    WebSocketOnEvent _OnEvent_Callfunc;
