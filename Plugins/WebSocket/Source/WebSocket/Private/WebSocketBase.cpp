...
UWebSocketBase::UWebSocketBase()
{
    ...
    _OnMessage_Callfunc = nullptr;
    _OnEvent_Callfunc = nullptr;
#endif
}
...
void UWebSocketBase::SendByteData(const char* data,uint32 len)
{
    ...
    if (mlws != nullptr && data) {
        unsigned char buf[LWS_PRE + MAX_ECHO_PAYLOAD];
        memset(&buf[LWS_PRE], 0, MAX_ECHO_PAYLOAD);
        unsigned char* sendBuf = &buf[LWS_PRE];
        memcpy(sendBuf, data, len);

        int sendReturn = lws_write(mlws, sendBuf, len, LWS_WRITE_BINARY);
        UE_LOG(WebSocket, Log, TEXT("send data status: %d"),sendReturn);
    }
...
void UWebSocketBase::SendDataBinary(MXBinaryStruct& data)
{
#if PLATFORM_UWP
    ...
    if (data.dataLen > MAX_ECHO_PAYLOAD)
    {
        UE_LOG(WebSocket, Error, TEXT("too large package to send > MAX_ECHO_PAYLOAD:%d > %d"), data.dataLen, MAX_ECHO_PAYLOAD);
        return;
    }

    if (mlws != nullptr )
    {
//        m_sendQueue_byte.push_back(data);
        m_SendQueue_byte.push_back(data);
    }
    else
    {
...
void UWebSocketBase::ProcessWriteable()
{
	...
	while (mSendQueue.Num() > 0)
	{
		...
		int sendRtn = lws_write(mlws, &buf[LWS_PRE], strData.size(), LWS_WRITE_TEXT);
        UE_LOG(WebSocket, Log, TEXT("send text status: %d"), sendRtn);

		mSendQueue.RemoveAt(0);
	}

    //write byte data
    while (m_SendQueue_byte.size() > 0)
    {
        MXBinaryStruct bs = m_SendQueue_byte.front();

        unsigned char buf[LWS_PRE + MAX_ECHO_PAYLOAD];
        memcpy(&buf[LWS_PRE], bs.sendData, bs.dataLen);

        int sendRtn = lws_write(mlws, &buf[LWS_PRE], bs.dataLen, LWS_WRITE_BINARY);
        UE_LOG(WebSocket, Log, TEXT("send binary data status: %d"), sendRtn);

        m_SendQueue_byte.pop_front();
    }
#endif
}
...
// 	OnReceiveData.Broadcast(strData);
    ...
    if (_OnMessage_Callfunc) {
        _OnMessage_Callfunc(in,(uint32)len);
    }
...
void UWebSocketBase::ProcessWebSocketEvent(EWS_State wsState ,const char* in, int len)
{
    if (_OnEvent_Callfunc) {
        _OnEvent_Callfunc(wsState,in,(uint32)len);
    }
}

void UWebSocketBase::SetWebSocketCallfunc(WebSocketOnMessage _Mes_callfunc, WebSocketOnEvent _Eve_callfunc)
{
    if (_Mes_callfunc) {
        _OnMessage_Callfunc = _Mes_callfunc;
    }

    if (_Eve_callfunc)
    {
        _OnEvent_Callfunc = _Eve_callfunc;
    }
}
